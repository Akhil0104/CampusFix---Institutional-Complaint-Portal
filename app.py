...
class User(UserMixin):
    def __init__(self, user_data):
        ...
        self.avatar = user_data.get('avatar', 'bi-person-circle')
...
def profile():
    if request.method == 'POST':
        ...
        avatar = request.form.get('avatar', 'bi-person-circle')
        ...
        existing_email = db.users.find_one({'email': email, '_id': {'$ne': ObjectId(current_user.id)}})
        if existing_email:
            flash('Email is already in use by another account', 'danger')
            return redirect(url_for('profile'))
            
        # Check if username is already taken
        existing_username = db.users.find_one({'username': username, '_id': {'$ne': ObjectId(current_user.id)}})
            ...
            db.users.update_one(
                {'_id': ObjectId(current_user.id)},
                {
                    '$set': {
                        ...
                        'phone': phone,
                        'department': department,
                        'roll_number': roll_number if current_user.role == 'student' else '',
                        'bio': bio,
                        'avatar': avatar
                    }
                }
    ...
    try:
        if current_user.role == 'student':
            activity_count = db.complaints.count_documents({'submitted_by': current_user.id})
            recent_activity = list(db.complaints.find({'submitted_by': current_user.id}).sort('created_at', -1).limit(5))
        elif current_user.role == 'staff':
            activity_count = db.complaints.count_documents({'assigned_to': current_user.id})
            recent_activity = list(db.complaints.find({'assigned_to': current_user.id}).sort('created_at', -1).limit(5))
        else:
            activity_count = db.complaints.count_documents({})
            recent_activity = list(db.complaints.find().sort('created_at', -1).limit(5))
    except Exception as e:
        activity_count = 0
        recent_activity = []
        
    return render_template('profile.html', activity_count=activity_count, recent_activity=recent_activity)
